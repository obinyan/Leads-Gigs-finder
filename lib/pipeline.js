...
import { scoreLead, draftProposal } from "./gemini.js";
...
export async function scoreAndMaybeDraft(leadId) {
  ...
  // No Gemini key configured: don't treat this as an error per lead. Leave
  // it as a plain unscored lead you can review manually, and it'll get
  // scored automatically once a key is added — no code changes needed then.
  if (!process.env.GEMINI_API_KEY) {
    await updateLead(leadId, {
      status: "unscored",
      scoreReason: "No Gemini API key set — showing this as a raw, unranked lead."
    });
    return;
  }
